...
func TestRFCVectors_GenerateKey(t *testing.T) {
	for _, vector := range testVectors {
		_, sk := GenerateKey(bytes.NewReader(vector.sk))

		assert.Equal(t, vector.sk, sk.nonce)
		assert.Equal(t, vector.pk, sk.pk)

		s, _ := edwards25519.NewScalar().SetBytesWithClamping(vector.x)
		assert.Equal(t, s, sk.sk)
	}
}

func TestRFCVectors_encodeToCurve(t *testing.T) {
	for _, vector := range testVectors {
		h := encodeToCurve(vector.pk, vector.alpha)
...
func TestRFCVectors_generateNonceHash(t *testing.T) {
	for _, vector := range testVectors {
		k := nonceGenerationBytes(vector.sk, vector.h)
...
func TestRFCVectors_Prove(t *testing.T) {
	for _, vector := range testVectors {
		_, sk := GenerateKey(bytes.NewReader(vector.sk))
		beta, proof := sk.Prove(vector.alpha)
