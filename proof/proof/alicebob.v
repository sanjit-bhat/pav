...
Lemma mk_servγ_full t γ x :
  server.Trust.get_full (mk_servγ t γ) = Some x → x = γ.
Proof. intros. destruct t; try done; by simplify_eq/=. Qed.

Lemma mk_servγ_sigpred t γ x :
  server.Trust.get_sigpred (mk_servγ t γ) = Some x → x = γ.(server.cfg.agreeγ).
Proof. intros. destruct t; try done; by simplify_eq/=. Qed.

Definition mk_adtrγ t γ :=
  match t with
...
Lemma mk_adtrγ_full t γ x :
  auditor.Trust.get_full (mk_adtrγ t γ) = Some x → x = γ.
Proof. intros. destruct t; try done; by simplify_eq/=. Qed.

Lemma mk_adtrγ_sigpred t γ x :
  auditor.Trust.get_sigpred (mk_adtrγ t γ) = Some x → x = γ.(auditor.cfg.agreeγ).
Proof. intros. destruct t; try done; by simplify_eq/=. Qed.
...
Proof.
  ...
  (* epoch 0. *)
  wp_apply (client.wp_New uidγs (mk_servγ serv_trust γ) with "[$Halice_good]") as "* @".
  { iFrame "#".
    iSplitR. { iPureIntro. by simplify_map_eq/=. }
    rewrite (assoc _).
    iSplitR.
    - destruct (server.Trust.get_full _) eqn:Ht; try done.
      apply mk_servγ_full in Ht. by subst.
    - destruct (server.Trust.get_sigpred _) eqn:Ht; try done.
      apply mk_servγ_sigpred in Ht. by subst. }
  clear Hblame.
  ...
  wp_apply (client.wp_New uidγs (mk_servγ serv_trust γ) with "[$Hbob_good]") as "* @".
  { iFrame "#".
    iSplitR. { iPureIntro. by simplify_map_eq/=. }
    rewrite (assoc _).
    iSplitR.
  ...
  wp_if_join
    ...
    "Hadtrs_ep0" ∷ (if servGood then True else
      ...
      "%Heq_servGood0" ∷ ⌜adtr0γ.(auditor.cfg.serv_good) = mk_servγ serv_trust γ⌝ ∗
      "%Heq_servGood1" ∷ ⌜adtr1γ.(auditor.cfg.serv_good) = mk_servγ serv_trust γ⌝ ∗
  ...
  { by iFrame "∗#". }
  { wp_apply (auditor.wp_New (mk_servγ serv_trust γ)) as "* @".
    { iFrame "#".
      destruct (server.Trust.get_full _) eqn:Ht; try done.
      apply mk_servγ_full in Ht. subst.
      by iFrame "#". }
    clear Hblame.
    wp_apply primitive.wp_Assume as "%".
    case_bool_decide as Ht; try done.
  ...
  wp_if_join
    ...
    "err" ∷ err_ptr ↦ ktcore.blame_to_u64 ∅ ∗
    "ep" ∷ ep_ptr ↦ x0 ∗
    "Hclient_al" ∷ client.Client.own γ0 ptr_c
                  {|
                  ...
                  |} ∗
    "Hclient_bob" ∷ client.Client.own γ1 ptr_c0
                   {|
                     client.state.epoch := uint.nat (W64 1);
                     client.state.keys := [None];
                     client.state.pend_pk := None
                   |} ∗
    "Hadtrs_ep1_0" ∷ (if servGood then True else
      ∃ adtr0γ adtr1γ,
      "#His_inv_adtr0" ∷ auditor.is_inv adtr0γ ∗
      "#His_inv_adtr1" ∷ auditor.is_inv adtr1γ ∗
      "Hadtr0" ∷ auditor.Auditor.lock_perm adtr0 adtr0γ ∗
      ...
      "#His_audit_al1" ∷ match auditor.Trust.get_sigpred (mk_adtrγ adtr_trust adtr0γ) with None => True | Some adtrγ =>
        ktcore.is_audit γ0.(client.cfg.agreeγ) adtrγ 1%nat end ∗
      "#His_audit_bob1" ∷ match auditor.Trust.get_sigpred (mk_adtrγ adtr_trust adtr1γ) with None => True | Some adtrγ =>
        ktcore.is_audit γ1.(client.cfg.agreeγ) adtrγ 1%nat end ∗
      "%Heq_startEp0" ∷ ⌜match auditor.Trust.get_sigpred (mk_adtrγ adtr_trust adtr0γ) with None => True | Some adtrγ =>
        (adtrγ.(ktcore.Agree.digs_start) + adtrγ.(ktcore.Agree.func_start) = 0)%nat end⌝ ∗
      "%Heq_startEp1" ∷ ⌜match auditor.Trust.get_sigpred (mk_adtrγ adtr_trust adtr1γ) with None => True | Some adtrγ =>
        (adtrγ.(ktcore.Agree.digs_start) + adtrγ.(ktcore.Agree.func_start) = 0)%nat end⌝)
  ...
  { iNamed "Hadtrs_ep0".
    ...
    { iFrame "#".
      ...
      { destruct (auditor.Trust.get_sigpred _) eqn:Ht; try done.
        apply mk_adtrγ_sigpred in Ht. subst.
        by iFrame "#". }
      { destruct (auditor.Trust.get_full _) eqn:Ht; try done.
        apply mk_adtrγ_full in Ht. subst.
  ...
  wp_if_join
    (λ v,
    ∃ (x0 : w64) adtr2 sl_adtr2Pk,
    "->" ∷ ⌜v = execute_val⌝ ∗
    "err" ∷ err_ptr ↦ ktcore.blame_to_u64 ∅ ∗
    "ep" ∷ ep_ptr ↦ x0 ∗
    "adtr2" ∷ adtr2_ptr ↦ adtr2 ∗
    "adtr2Pk" ∷ adtr2Pk_ptr ↦ sl_adtr2Pk ∗
    "Hadtrs_ep1_1" ∷ (if servGood then True else
      ∃ adtr2γ,
      "#His_inv_adtr2" ∷ auditor.is_inv adtr2γ ∗
      "Hadtr2" ∷ auditor.Auditor.lock_perm adtr2 adtr2γ ∗
      "%Heq_servGood2" ∷ ⌜adtr2γ.(auditor.cfg.serv_good) = mk_servγ serv_trust γ⌝ ∗
      "%Heq_servPk2" ∷ ⌜adtr2γ.(auditor.cfg.serv_sig_pk) = γ.(server.cfg.sig_pk)⌝ ∗
      "#Hsl_sigPk2" ∷ sl_adtr2Pk ↦*□ adtr2γ.(auditor.cfg.adtr_sig_pk) ∗
      "#His_sigPk2" ∷ cryptoffi.is_sig_pk adtr2γ.(auditor.cfg.adtr_sig_pk)
        (sigpred.P adtr2γ.(auditor.cfg.agreeγ)))
    )%I with "[err ep adtr2 adtr2Pk]".
  { by iFrame "∗#". }
  { wp_apply (auditor.wp_New (mk_servγ serv_trust γ)) as "* @".
    { iFrame "#".
      destruct (server.Trust.get_full _) eqn:Ht; try done.
    ...
    apply ktcore.rw_Blame0 in Ht. subst.
    destruct (decide (_ ≠ ∅)) as [Ht|Ht]; try done. clear Ht.
    iNamedSuffix "Herr" "_adtr2".
    replace (Z.to_nat rwmutex.actualMaxReaders) with
      (S $ pred $ Z.to_nat rwmutex.actualMaxReaders).
    2: { rewrite rwmutex.actualMaxReaders_unseal. lia. }
    simpl.
    iDestruct "Hlocks_adtr2" as "[Hadtr2 Hlocks_adtr2]".
    case_decide as Ht; [|word]. clear Ht.
    wp_apply (wp_load_slice_index with "[$Hsl_adtrAddrs]") as "_"; [word|done|].
    wp_apply (auditor.wp_NewRpcServer with "[$Hlocks_adtr2]") as "* H".
    iNamedSuffix "H" "2".
    wp_apply advrpc.wp_Server_Serve.
    { iFrame "#". solve_pkg_init2. }
    wp_apply time.wp_Sleep.
    iFrame "adtr2 adtr2Pk".
    by iFrame "∗#". }
  iIntros "* @". wp_auto.
  iPersist "adtr2 adtr2Pk".

  (* epoch 2. *)
  wp_apply cryptoffi.wp_RandBytes as "%sl_alicePk2 %alicePk2 @".
  clear Hlen_b. iRename "Hsl_b" into "Hsl_alicePk2".
  iPersist "Hsl_alicePk2".
  wp_apply (client.wp_Client_Put with "[$Hclient_al]") as "@".
  { iFrame "#". }
  wp_apply time.wp_Sleep.
  wp_apply (client.wp_Client_SelfMon with "[$Hclient]") as "* @".
  ...
  apply ktcore.rw_Blame0 in Ht. subst.
  destruct (decide (_ ≠ ∅)) as [Ht|Ht]; try done. clear Ht.
  iNamedSuffix "Herr" "_bob2". simpl in *.
  wp_apply primitive.wp_Assume as "%".
  case_bool_decide as Ht; try done. subst.
  ...
  wp_if_join
    ...
    "Hclient_al" ∷ client.Client.own γ0 ptr_c
                  {|
                    client.state.epoch := uint.nat (W64 2);
                    client.state.keys :=
                      None
                      :: (replicate num_last_key last_key ++
                          Some alicePk1
                          :: replicate num_pend_key (Some alicePk1)) ++
                         replicate num_last_key0 last_key0 ++
                         Some alicePk2
                         :: replicate num_pend_key0 (Some alicePk2);
                    client.state.pend_pk := None
                  |} ∗
    "Hclient_bob" ∷ client.Client.own γ1 ptr_c0
                   {|
                     ...
                     client.state.pend_pk := None
                   |} ∗
    "Hadtrs_ep2" ∷ (if servGood then True else
      ∃ adtr2γ,
      "#His_inv_adtr2" ∷ auditor.is_inv adtr2γ ∗
      "Hadtr2" ∷ auditor.Auditor.lock_perm adtr2 adtr2γ ∗
      "%Heq_servGood2" ∷ ⌜adtr2γ.(auditor.cfg.serv_good) = mk_servγ serv_trust γ⌝ ∗
      ...
      "#His_audit_al2" ∷ match auditor.Trust.get_sigpred (mk_adtrγ adtr_trust adtr2γ) with None => True | Some adtrγ =>
        ktcore.is_audit γ0.(client.cfg.agreeγ) adtrγ 2%nat end ∗
      "#His_audit_bob2" ∷ match auditor.Trust.get_sigpred (mk_adtrγ adtr_trust adtr2γ) with None => True | Some adtrγ =>
        ktcore.is_audit γ1.(client.cfg.agreeγ) adtrγ 2%nat end ∗
      "%Heq_startEp" ∷ ⌜match auditor.Trust.get_sigpred (mk_adtrγ adtr_trust adtr2γ) with None => True | Some adtrγ =>
        (adtrγ.(ktcore.Agree.digs_start) + adtrγ.(ktcore.Agree.func_start) ≤ 1)%nat end⌝)
    )%I with "[err ep Hclient_al2 Hclient_bob2 Hadtrs_ep1_1]".
  { by iFrame "∗#". }
  { iNamed "Hadtrs_ep1_1".
    wp_apply (auditor.wp_Auditor_Update (λ _, True%I) with "[$Hadtr2]") as "* H".
    { by iApply auditor.op_update. }
    iNamedSuffix "H" "2".
    clear Hblame2.
    case_decide as Ht; [|word]. clear Ht.
    ...
    { iFrame "#".
      ...
      { destruct (auditor.Trust.get_full _) eqn:Ht; try done.
        apply mk_adtrγ_full in Ht. subst.
        iPureIntro. split; [done|congruence]. } }
    iNamedSuffix "H" "_bob2".
    clear Hblame_bob2.
    wp_apply primitive.wp_Assume as "%".
    case_bool_decide as Ht; try done.
    apply ktcore.rw_Blame0 in Ht. subst.
    destruct (decide (_ ≠ ∅)) as [Ht|Ht]; try done. clear Ht.
    iNamedSuffix "Herr_bob2" "_bob2".
    wp_apply primitive.wp_Assume as "%".
    case_bool_decide as Ht; try done. subst.
    wp_apply primitive.wp_Assume as "%".
    case_bool_decide as Ht; try done. subst.
    replace (uint.nat (W64 2)) with 2%nat by word.
    by iFrame "∗#". }
  iIntros "* @". wp_auto.

  (* KT agreement. *)
  autorewrite with len in *.
  replace num_last_key with 0%nat by word.
  replace num_pend_key with 0%nat by word.
  replace num_last_key0 with 0%nat by word.
  replace num_pend_key0 with 0%nat by word.
  replace (uint.nat (W64 0)) with 0%nat by word.
  replace (uint.nat (W64 1)) with 1%nat by word.
  replace (uint.nat (W64 2)) with 2%nat by word.
  iDestruct (client.know_keys with "Hclient_al") as "#Hstaged". simpl.

  iAssert (
    ∃ adtrγ,
    "#His_audit_al" ∷ ktcore.is_audit γ0.(client.cfg.agreeγ) adtrγ 2 ∗
    "#His_audit_bob" ∷ ktcore.is_audit γ1.(client.cfg.agreeγ) adtrγ 2 ∗
    "%Heq_func_start" ∷ ⌜adtrγ.(ktcore.Agree.func_start) = 0%nat⌝
  )%I with "[Hclient_al Hclient_bob Hadtrs_ep1_0 Hadtrs_ep2]" as "@".
  { destruct servGood eqn:Ht; subst servGood;
      case_bool_decide; try done; clear Ht.
    - destruct (server.Trust.get_sigpred (mk_servγ serv_trust γ)) eqn:Heq_good.
      2: { by destruct serv_trust. }
      pose proof Heq_good as Ht.
      apply mk_servγ_sigpred in Ht as ->.
      iDestruct (client.serv_is_adtr with "Hclient_al") as "[$ %]".
      { by rewrite Heq_serv_good_al. }
      iDestruct (client.serv_is_adtr with "Hclient_bob") as "[$ %]".
      { by rewrite Heq_serv_good_bob. }
      word.
    - iNamed "Hadtrs_ep1_0". iNamed "Hadtrs_ep2".
      destruct (auditor.Trust.get_sigpred (mk_adtrγ adtr_trust adtr0γ)) eqn:Heq_good0.
      2: { destruct serv_trust, adtr_trust; try done. lia. }
      pose proof Heq_good0 as Ht.
      apply mk_adtrγ_sigpred in Ht as ->.
      destruct (auditor.Trust.get_sigpred (mk_adtrγ adtr_trust adtr1γ)) eqn:Heq_good1.
      2: { by destruct adtr_trust. }
      pose proof Heq_good1 as Ht.
      apply mk_adtrγ_sigpred in Ht as ->.
      destruct (auditor.Trust.get_sigpred (mk_adtrγ adtr_trust adtr2γ)) eqn:Heq_good2.
      2: { by destruct adtr_trust. }
  ...
  simplify_eq/=.
  iDestruct (ktcore.commit_staged with "Hstaged His_audit_al") as "H".
  { word. }
  { simpl. word. }
  replace (γ0.(client.cfg.agreeγ).(ktcore.Agree.digs_start) +
    γ0.(client.cfg.agreeγ).(ktcore.Agree.func_start))%nat with 0%nat by word.
  simpl.
  iPoseProof ("H" $! 0%nat with "[]") as "Hal0"; [done|].
  ...
  iDestruct (ktcore.kt_ptsto_agree with "Hal2 Hbob2") as %<-; [done..|].
  iNamedSuffix "Hopt_pk_bob0" "0".
  iNamedSuffix "Hopt_pk_bob1" "1".
  iNamedSuffix "Hopt_pk_bob2" "2".
  subst.
  wp_apply std.wp_Assert; [done|].
  wp_apply std.wp_Assert; [done|].
  wp_apply bytes.wp_Equal as "_".
  { iFrame "#". }
  wp_apply std.wp_Assert; [by case_bool_decide|].
  wp_apply std.wp_Assert; [done|].
  wp_apply bytes.wp_Equal as "_".
  { iFrame "#". }
  wp_apply std.wp_Assert; [by case_bool_decide|].
