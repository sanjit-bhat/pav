...
Proof.
  ...
  - iDestruct (own_slice_dfrac_valid with "Hsl_b") as %Hvd; [simpl; lia|].
    ...
    + (* ReadInt failed *)
      wp_auto. iApply "HΦ". iIntros "Hex". iDestruct "Hex" as (obj tail) "%Hwish".
      destruct Hwish as [Henc _]. iApply "Hpost1".
      iExists (W64 (length obj)), (mjoin (UpdateProof.pure_enc <$> obj) ++ tail). iPureIntro.
      rewrite /safemarshal.w64.wish /safemarshal.w64.pure_enc Henc /pure_enc
        /safemarshal.w64.pure_enc -app_assoc //.
    + iDestruct "Hpost1" as (tail1) "(Hb1 & %Hwish1)".
      wp_auto.
      ...
      * (* length0 >= 0 : decode loop *)
        ...
        iIntros (lo_sl) "(Hlo & Hcap & %Hlocap)".
        wp_auto.
      iAssert (∃ (j : w64) (decoded : list UpdateProof.t) (ptrs : list loc)
                 (cur_lo cur_lb : slice.t) (rest : list w8),
        "i" ∷ i_ptr ↦ j ∗
        "%Hj" ∷ ⌜uint.Z j ≤ uint.Z length0⌝ ∗
        "%Hdeclen" ∷ ⌜Z.of_nat (length decoded) = uint.Z j⌝ ∗
        ...
        "Hlo" ∷ cur_lo ↦* ptrs ∗
        "Hcap" ∷ own_slice_cap loc cur_lo (DfracOwn 1) ∗
        "Hbig" ∷ ([∗ list] p;o ∈ ptrs;decoded, UpdateProof.own p o d) ∗
        "loopErr" ∷ loopErr_ptr ↦ false ∗
        "loopB" ∷ loopB_ptr ↦ cur_lb ∗
        "Hlb" ∷ cur_lb ↦*{d} rest ∗
        "%Hcons" ∷ ⌜tail1 = mjoin (UpdateProof.pure_enc <$> decoded) ++ rest⌝ ∗
        "%Hvf" ∷ ⌜Forall UpdateProof.valid decoded⌝)%I
        with "[i loopO Hlo Hcap loopErr loopB Hb1]" as "IH".
      { assert (Hrep : replicate (sint.nat (W64 0)) (zero_val loc) = (@nil loc)).
        { replace (sint.nat (W64 0)) with 0%nat by word. done. }
        iEval (rewrite Hrep) in "Hlo".
      ...
      (* body: uint.Z j < uint.Z length0 *)
      wp_auto.
      wp_apply (UpdateProof.wp_dec with "[$Hlb]").
      iIntros (a2 lb1 err2) "Hpost2".
      wp_auto.
      destruct err2.
      -- (* Memb decode failed → set loopErr, break, return error *)
         ...
         { intros a0 a1 s0 s1 Hva0 Hva1 He.
           apply (UpdateProof.wish_det s0 s1 a0 a1 (b := UpdateProof.pure_enc a0 ++ s0));
             rewrite /UpdateProof.wish; by split. }
         rewrite Hcons in Hrest_eq.
         assert (Hmjobj : mjoin (UpdateProof.pure_enc <$> obj)
                          = mjoin (UpdateProof.pure_enc <$> take (uint.nat j) obj)
                            ++ UpdateProof.pure_enc oj
                            ++ mjoin (UpdateProof.pure_enc <$> drop (S (uint.nat j)) obj)).
         { rewrite -{1}Hsplit fmap_app fmap_cons join_app join_cons -app_assoc //. }
         rewrite Hmjobj -!app_assoc in Hrest_eq.
         assert (Hleneq : length decoded = length (take (uint.nat j) obj)).
         { rewrite length_take Nat.min_l; [word | lia]. }
         assert (Hforalltake : Forall UpdateProof.valid (take (uint.nat j) obj))
           by (apply Forall_take; exact Hvforall).
         destruct (mjoin_enc_inj _ _ _ _ _ _ Hinj Hleneq Hvf Hforalltake Hrest_eq)
           as [_ Hrest_split].
         iApply "Hno".
         iExists oj, (mjoin (UpdateProof.pure_enc <$> drop (S (uint.nat j)) obj) ++ tail).
         iPureIntro. rewrite /UpdateProof.wish. split.
         { rewrite Hrest_split /UpdateProof.pure_enc /safemarshal.Slice1D.pure_enc -!app_assoc //. }
         rewrite Forall_forall in Hvforall.
         apply Hvforall. by eapply list_elem_of_lookup_2.
      -- (* success: append the decoded element and continue *)
         iDestruct "Hpost2" as (m t) "(Hown_m & Hlb1 & %Hwm)".
         ...
         iIntros (lo2) "(Hlo & Hcap & _)".
         wp_auto.
         iAssert ([∗ list] p;o ∈ (ptrs ++ [a2]);(decoded ++ [m]), UpdateProof.own p o d)%I
           with "[Hbig Hown_m]" as "Hbig2".
         { iApply big_sepL2_snoc. iFrame. }
         assert (Hcons' : tail1 = mjoin (UpdateProof.pure_enc <$> (decoded ++ [m])) ++ t).
         { rewrite fmap_app join_app /= app_nil_r Hcons Hrest_eq -app_assoc //. }
         assert (Hvf' : Forall UpdateProof.valid (decoded ++ [m])).
         { apply Forall_app; split; [exact Hvf|by apply Forall_singleton]. }
         assert (Hj' : uint.Z (word.add j (W64 1)) ≤ uint.Z length0) by word.
         assert (Hdeclen' : Z.of_nat (length (decoded ++ [m])) = uint.Z (word.add j (W64 1))).
         { rewrite length_app /=. word. }
...
Proof.
  ...
  - iDestruct (own_slice_dfrac_valid with "Hsl_b") as %Hvd; [simpl; lia|].
    ...
    + (* ReadInt failed *)
      wp_auto. iApply "HΦ". iIntros "Hex". iDestruct "Hex" as (obj tail) "%Hwish".
      destruct Hwish as [Henc _]. iApply "Hpost1".
      iExists (W64 (length obj)), (mjoin (AuditProof.pure_enc <$> obj) ++ tail). iPureIntro.
      rewrite /safemarshal.w64.wish /safemarshal.w64.pure_enc Henc /pure_enc
        /safemarshal.w64.pure_enc -app_assoc //.
    + iDestruct "Hpost1" as (tail1) "(Hb1 & %Hwish1)".
      wp_auto.
      ...
      * (* length0 >= 0 : decode loop *)
        ...
        iIntros (lo_sl) "(Hlo & Hcap & %Hlocap)".
        wp_auto.
      iAssert (∃ (j : w64) (decoded : list AuditProof.t) (ptrs : list loc)
                 (cur_lo cur_lb : slice.t) (rest : list w8),
        "i" ∷ i_ptr ↦ j ∗
        "%Hj" ∷ ⌜uint.Z j ≤ uint.Z length0⌝ ∗
        "%Hdeclen" ∷ ⌜Z.of_nat (length decoded) = uint.Z j⌝ ∗
        ...
        "Hlo" ∷ cur_lo ↦* ptrs ∗
        "Hcap" ∷ own_slice_cap loc cur_lo (DfracOwn 1) ∗
        "Hbig" ∷ ([∗ list] p;o ∈ ptrs;decoded, ktcore.AuditProof.own p o d) ∗
        "loopErr" ∷ loopErr_ptr ↦ false ∗
        "loopB" ∷ loopB_ptr ↦ cur_lb ∗
        "Hlb" ∷ cur_lb ↦*{d} rest ∗
        "%Hcons" ∷ ⌜tail1 = mjoin (AuditProof.pure_enc <$> decoded) ++ rest⌝ ∗
        "%Hvf" ∷ ⌜Forall AuditProof.valid decoded⌝)%I
        with "[i loopO Hlo Hcap loopErr loopB Hb1]" as "IH".
      { assert (Hrep : replicate (sint.nat (W64 0)) (zero_val loc) = (@nil loc)).
        { replace (sint.nat (W64 0)) with 0%nat by word. done. }
        iEval (rewrite Hrep) in "Hlo".
      ...
      (* body: uint.Z j < uint.Z length0 *)
      wp_auto.
      wp_apply (AuditProof.wp_dec with "[$Hlb]").
      iIntros (a2 lb1 err2) "Hpost2".
      wp_auto.
      destruct err2.
      -- (* Memb decode failed → set loopErr, break, return error *)
         ...
         { intros a0 a1 s0 s1 Hva0 Hva1 He.
           apply (AuditProof.wish_det s0 s1 a0 a1 (b := AuditProof.pure_enc a0 ++ s0));
             rewrite /AuditProof.wish; by split. }
         rewrite Hcons in Hrest_eq.
         assert (Hmjobj : mjoin (AuditProof.pure_enc <$> obj)
                          = mjoin (AuditProof.pure_enc <$> take (uint.nat j) obj)
                            ++ AuditProof.pure_enc oj
                            ++ mjoin (AuditProof.pure_enc <$> drop (S (uint.nat j)) obj)).
         { rewrite -{1}Hsplit fmap_app fmap_cons join_app join_cons -app_assoc //. }
         rewrite Hmjobj -!app_assoc in Hrest_eq.
         assert (Hleneq : length decoded = length (take (uint.nat j) obj)).
         { rewrite length_take Nat.min_l; [word | lia]. }
         assert (Hforalltake : Forall AuditProof.valid (take (uint.nat j) obj))
           by (apply Forall_take; exact Hvforall).
         destruct (mjoin_enc_inj _ _ _ _ _ _ Hinj Hleneq Hvf Hforalltake Hrest_eq)
           as [_ Hrest_split].
         iApply "Hno".
         iExists oj, (mjoin (AuditProof.pure_enc <$> drop (S (uint.nat j)) obj) ++ tail).
         iPureIntro. rewrite /AuditProof.wish. split.
         { rewrite Hrest_split /AuditProof.pure_enc /UpdateProofSlice1D.pure_enc
             /safemarshal.Slice1D.pure_enc -!app_assoc //. }
         rewrite Forall_forall in Hvforall.
         apply Hvforall. by eapply list_elem_of_lookup_2.
      -- (* success: append the decoded element and continue *)
         ...
         iIntros (lo2) "(Hlo & Hcap & _)".
         wp_auto.
         iAssert ([∗ list] p;o ∈ (ptrs ++ [a2]);(decoded ++ [m]), ktcore.AuditProof.own p o d)%I
           with "[Hbig Hown_m]" as "Hbig2".
         { iApply big_sepL2_snoc. iFrame. }
         assert (Hcons' : tail1 = mjoin (AuditProof.pure_enc <$> (decoded ++ [m])) ++ t).
         { rewrite fmap_app join_app /= app_nil_r Hcons Hrest_eq -app_assoc //. }
         assert (Hvf' : Forall AuditProof.valid (decoded ++ [m])).
         { apply Forall_app; split; [exact Hvf|by apply Forall_singleton]. }
         assert (Hj' : uint.Z (word.add j (W64 1)) ≤ uint.Z length0) by word.
         assert (Hdeclen' : Z.of_nat (length (decoded ++ [m])) = uint.Z (word.add j (W64 1))).
         { rewrite length_app /=. word. }
