...
func newRPCServer(s *Server) *advrpc.Server {
	...
	h[ServerPutRpc] = func(arg []byte, reply *[]byte) {
		...
		ret0 := s.Put(argObj.Uid, argObj.Pk)
		replyObj := &ServerPutReply{P: ret0}
	...
	h[ServerGetRpc] = func(arg []byte, reply *[]byte) {
		...
		ret0 := s.Get(argObj.Uid)
		replyObj := &ServerGetReply{P: ret0}
...
func callServPut(c *advrpc.Client, uid uint64, pk []byte) (*HistProof, bool) {
...
func callServGet(c *advrpc.Client, uid uint64) (*HistProof, bool) {
